...
import com.ddona.mvvm.BuildConfig
import com.securepreferences.SecurePreferences

class AppSharedPreference constructor(app: Context) {

    private val preference = if (BuildConfig.DEBUG) {
        PreferenceManager.getDefaultSharedPreferences(app)
    } else {
        SecurePreferences(app, BuildConfig.APPLICATION_ID, "app_preference_file")
    }
    ...
    fun getString(key: String, default: String? = null): String? =
        kotlin.runCatching { preference.getString(key, default) }.getOrDefault(default)

    fun getInt(key: String, default: Int = 0): Int =
        kotlin.runCatching { preference.getInt(key, default) }.getOrDefault(default)

    fun getFloat(key: String, default: Float = 0f): Float =
        kotlin.runCatching { preference.getFloat(key, default) }.getOrDefault(default)

    fun getLong(key: String, default: Long = 0L): Long =
        kotlin.runCatching { preference.getLong(key, default) }.getOrDefault(default)

    fun getBoolean(key: String, default: Boolean = false): Boolean =
        kotlin.runCatching { preference.getBoolean(key, default) }.getOrDefault(default)
